...
export async function verifyPayment(txnid: string) {
  ...
  const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body, cache: "no-store", signal: AbortSignal.timeout(8_000) });
  if (!response.ok) throw new Error("PayU verification failed");
  const payload = await response.json() as { transaction_details?: Record<string, unknown> };
  return normalizeVerification(payload.transaction_details?.[txnid]);
}
